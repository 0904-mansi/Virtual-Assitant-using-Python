...
#########################
ai_name = 'Virtual Assistance'.lower()
...
with open('E:\shared folder\db.json') as file:
...
if __name__ == '__main__':
	...
	def Arc_clicked(event):
		...
		entry2.place(x=130, y=58)
		button.place(x=95,y=120)
		def reload():
			print("reloading it")
			data2.close()
			with open('E:\shared folder\db.json') as file:
		...
		def add_clicked(event):
			...
			key=entry2.get()
			print(key)
			add_element={phrase:key}
			data2.update(add_element)
			with open('E:\shared folder\db.json', "w") as outfile:
